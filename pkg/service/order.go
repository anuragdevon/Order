...
func (os *OrderService) CreateOrder(ctx context.Context, req *pb.CreateOrderRequest) (*pb.CreateOrderResponse, error) {
	...
	if err != nil {
		...
		return &pb.CreateOrderResponse{Status: itemResp.Status, Error: itemResp.Error}, nil
	} else if itemResp.Data.Quantity < req.Quantity {
		return &pb.CreateOrderResponse{Status: http.StatusConflict, Error: "Quantity is insufficient"}, nil
	}
